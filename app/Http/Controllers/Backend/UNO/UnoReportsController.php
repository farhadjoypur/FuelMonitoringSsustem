...
class UnoReportsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($unoUpazila) $request->merge(['thana_upazila' => $unoUpazila]);
        if ($unoDistrict) $request->merge(['district' => $unoDistrict]);

        $hasAnyFilter = $this->hasAnyFilterApplied($request);

        $stationsQuery = FillingStation::orderBy('station_name');
        if ($unoUpazila) $stationsQuery->where('upazila', $unoUpazila);
        if ($unoDistrict) $stationsQuery->where('district', $unoDistrict);
    ...
    private function buildFilteredQuery(Request $request)
    {
        ...
        if ($unoUpazila) $query->where('thana_upazila', $unoUpazila);
        if ($unoDistrict) $query->where('district', $unoDistrict);
        ...
        if ($request->filled('station_id')) {
            ...
            if ($unoUpazila) $stationQuery->where('upazila', $unoUpazila);
            if ($unoDistrict) $stationQuery->where('district', $unoDistrict);
            if ($stationQuery->exists()) $query->where('station_id', $request->station_id);
        }
        ...
        if ($request->filled('stock_status')) {
            ...
            $query->where(function ($q) use ($status) {
                ...
                $sumExpr  = implode(' + ', array_map(fn($f) => "COALESCE({$f}_closing_stock, 0)", $fuelKeys));
                match ($status) {
    ...
    private function loadOfficerMap(?string $unoUpazila, ?string $unoDistrict): Collection
    {
        return AssignTagOfficer::with(['officer.profile', 'fillingStation'])
            ...
            ->whereHas('fillingStation', function ($q) use ($unoUpazila, $unoDistrict) {
                if ($unoUpazila) $q->where('upazila', $unoUpazila);
                if ($unoDistrict) $q->where('district', $unoDistrict);
            })
            ->get()
    ...
    public function destroy(int $id)
    {
        ...
        if ($unoUpazila && $report->thana_upazila !== $unoUpazila) $authorized = false;
        if ($unoDistrict && $report->district !== $unoDistrict) $authorized = false;
    ...
    public function differenceReport(Request $request)
    {
        ...
        if (!empty($validated['station_id'])) {
            ...
            if ($unoUpazila) $stationQuery->where('upazila', $unoUpazila);
            if ($unoDistrict) $stationQuery->where('district', $unoDistrict);
            if ($stationQuery->exists()) $query->where('station_id', $validated['station_id']);
        }

        if (!empty($validated['tag_officer'])) {
            $query->whereHas(
                'fillingStation.assignedOfficer.officer.profile',
                fn($q) => $q->where('name', 'like', "%{$validated['tag_officer']}%")
            );
        }
   ...
   public function missingReport(Request $request)
    {
        ...
        // Build per-day reported map scoped to UNO jurisdiction
    ...
    public function edit(Fuelreport $fuelReport)
    {
        ...
        // Security: UNO can only edit reports from their own upazila
        if ($unoUpazila && strtolower($fuelReport->thana_upazila) !== strtolower($unoUpazila)) {
            abort(403, 'You are not authorized to edit this report.');
        }
        if ($unoDistrict && strtolower($fuelReport->district) !== strtolower($unoDistrict)) {
            abort(403, 'You are not authorized to edit this report.');
        }
        ...
        return view('backend.uno.pages.reports.edit', compact('fuelReport', 'tagOfficerName'));
    }
    ...
    public function update(Request $request, Fuelreport $fuelReport)
    {
        ...
        // Security: UNO can only update reports from their own upazila
        if ($unoUpazila && strtolower($fuelReport->thana_upazila) !== strtolower($unoUpazila)) {
        ...
        return redirect()
            ->route('uno.reports.edit', $fuelReport)
    ...
    public function exportPdf(Request $request)
    {
        ...
        return view('backend.uno.pages.reports.pdf_template', [
    ...
    public function exportDifferencePdf(Request $request)
    {
        ...
        return view('backend.uno.pages.reports.pdf_difference', [
            'rows'    => $rows,
            'filters' => $filters,
        ]);
    }
    ...
    public function exportMissingPdf(Request $request)
    {
        ...
        return view('backend.uno.pages.reports.pdf_missing', [
            'rows'     => $rows,
            'filters'  => $filters,
    ...
    public function exportSubmittedPdf(Request $request)
    {
        ...
        return view('backend.uno.pages.reports.pdf_submitted', [
            'rows'    => $rows,
            'filters' => $filters,
        ]);
    }
    ...
    public function exportCsv(Request $request)
    {
        ...
        return $this->streamCsv('uno-stock-sales-' . now()->format('Y-m-d') . '.csv', $headers, $rows);
    }

    // ─────────────────────────────────────────────────────────────
    // EXPORT CSV — DIFFERENCE REPORT
    // ─────────────────────────────────────────────────────────────
    ...
    public function exportDifferenceCsv(Request $request)
    {
        ...
        $officerMap    = AssignTagOfficer::with(['officer.profile'])->where('status', 'active')->get()->keyBy('filling_station_id');
        $allRawReports = $query->orderBy('report_date', 'desc')->orderBy('station_id')->get();

        $headers = ['#', 'Date', 'Station', 'Company', 'Tag Officer', 'Designation', 'Phone', 'District', 'Upazila', 'Fuel', 'Diff (L)', 'Diff (%)', 'Status'];
        ...
        foreach ($allRawReports as $report) {
            ...
            $fuelData = [];
            foreach ($fuelTypes as $fuel) {
                $supply     = (float)($report->{"{$fuel}_supply"} ?? 0);
                $received   = (float)($report->{"{$fuel}_received"} ?? 0);
                $diffL      = $supply - $received;
                $diffPct    = $supply > 0 ? round(($diffL / $supply) * 100, 2) : 0;
                $diffStatus = match(true) {
                    abs($diffPct) >= 5 => 'High',
                    abs($diffPct) >= 1 => 'Low',
                    default            => 'Normal',
                };
                $fuelData[] = ['fuel' => ucfirst($fuel), 'diffL' => number_format($diffL, 0), 'diffPct' => $diffPct, 'status' => $diffStatus];
            }

            // Apply filters
            if ($request->filled('min_diff_l')) {
                $minL = (float)$request->min_diff_l;
                if (!collect($fuelData)->contains(fn($f) => abs((float)str_replace(',', '', $f['diffL'])) >= $minL)) continue;
            }
            if ($request->filled('min_diff_pct')) {
                $minPct = (float)$request->min_diff_pct;
                if (!collect($fuelData)->contains(fn($f) => abs($f['diffPct']) >= $minPct)) continue;
            }
            if ($request->filled('diff_status')) {
                $target = ucfirst($request->diff_status);
                if (!collect($fuelData)->contains(fn($f) => $f['status'] === $target)) continue;
            }
            ...
            foreach ($fuelData as $fuel) {
                $rows[] = [
                    $firstRow ? $serial : '',
                    $firstRow ? ("\t" . \Carbon\Carbon::parse($report->report_date)->format('d M Y')) : '',
                    $firstRow ? ($report->station_name ?? '—') : '',
                    $firstRow ? ($report->fillingStation?->company?->code ?? '—') : '',
                    $firstRow ? $tagOfficerName : '',
                    $firstRow ? $officerDesignation : '',
                    $firstRow ? ("\t" . $officerPhone) : '',
                    $firstRow ? ($report->district ?? '—') : '',
                    $firstRow ? ($report->thana_upazila ?? '—') : '',
                    $fuel['fuel'],
                    $fuel['diffL'],
                    $fuel['diffPct'] . '%',
                    $fuel['status'],
        ...
        return $this->streamCsv('uno-difference-' . now()->format('Y-m-d') . '.csv', $headers, $rows);
    }

    // ─────────────────────────────────────────────────────────────
    // EXPORT CSV — MISSING REPORT
    // ─────────────────────────────────────────────────────────────
    ...
    public function exportMissingCsv(Request $request)
    {
        ...
        $assignmentsQuery = AssignTagOfficer::with([
            'officer.profile', 'fillingStation.company', 'fillingStation.depot',
        ])->where('status', 'active')
        ->whereHas('fillingStation', function ($q) use ($unoUpazila, $unoDistrict) {
            if ($unoUpazila)  $q->where('upazila', $unoUpazila);
            if ($unoDistrict) $q->where('district', $unoDistrict);
        ...
        $headers = ['#', 'Missing Date', 'Officer Name', 'Phone', 'Division', 'District', 'Upazila', 'Filling Station', 'Company', 'Depot', 'Status'];
        $rows    = collect();
        $serial  = 1;
        $current = $fromDate->copy();
            ...
            foreach ($allAssignments as $assignment) {
                if (!in_array($assignment->filling_station_id, $reportedTodayIds)) {
                    ...
                    $rows->push([
                        ...
                        $station?->company?->code ?? '—',
                        $station?->depot?->depot_name ?? '—',
                        'Pending',
                    ]);
                }
            }
        ...
        return $this->streamCsv('uno-missing-' . now()->format('Y-m-d') . '.csv', $headers, $rows);
    }

    // ─────────────────────────────────────────────────────────────
    // EXPORT CSV — SUBMITTED REPORT
    // ─────────────────────────────────────────────────────────────
    ...
    public function exportSubmittedCsv(Request $request)
    {
        ...
        $headers = ['#', 'Submit Date', 'Officer Name', 'Phone', 'Division', 'District', 'Upazila', 'Station', 'Company', 'Depot', 'Octane Closing', 'Petrol Closing', 'Diesel Closing', 'Others Closing', 'Status'];
        ...
        $rows = $allReports->map(function ($report, $index) use ($fuelTypes, $officerMap) {
            ...
            $row = [
                ...
                $officerProfile?->name ?? $assignment?->officer?->name ?? '—',
                "\t" . ($officerProfile?->phone ?? $assignment?->officer?->phone ?? '—'),
                $report->fillingStation?->division ?? '—',
                $report->district ?? '—',
                $report->thana_upazila ?? '—',
                $report->station_name ?? '—',
                $report->fillingStation?->company?->code ?? '—',
                $report->depot_name ?? $report->fillingStation?->depot?->depot_name ?? '—',
            ];

            foreach ($fuelTypes as $fuel) {
                $row[] = number_format((float)($report->{"{$fuel}_closing_stock"} ?? 0), 0);
            }
            $row[] = 'Submitted';
        ...
        return $this->streamCsv('uno-submitted-' . now()->format('Y-m-d') . '.csv', $headers, $rows);
    }
}

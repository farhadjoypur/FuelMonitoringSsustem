...
{
    protected $dcDistrict;

    public function __construct()
    {
        $this->dcDistrict = Auth::user()?->district
            ?? Auth::user()?->profile?->district
            ?? null;

        if (empty($this->dcDistrict)) {
    ...
    public function index(Request $request)
    {
        ...
        if (! $hasAnyFilter) {
            ...
            $tableHtml = view('backend.dc.pages.reports.table', [
                'reports'     => collect(),
                'totalRow'    => null,
                'currentPage' => 1,
                'lastPage'    => 1,
    ...
    {
        // Security: DC can only edit reports from their own district
        if (strtolower($fuelReport->district) !== strtolower($this->dcDistrict)) {
        ...
        return view('backend.dc.pages.reports.edit', compact('fuelReport', 'tagOfficerName'));
    }

    public function update(Request $request, Fuelreport $fuelReport)
    {
        // Security: DC can only update reports from their own district
        if (strtolower($fuelReport->district) !== strtolower($this->dcDistrict)) {
        ...
        return redirect()
            ->route('dc.reports.edit', $fuelReport)
    ...
    public function exportPdf(Request $request)
    {
        ...
        return view('backend.dc.pages.reports.pdf_template', [
    ...
    public function exportDifferencePdf(Request $request)
    {
        ...
        return view('backend.dc.pages.reports.pdf_difference', [
            'rows'    => $rows,
            'filters' => $filters,
        ]);
    }
    ...
    public function exportMissingPdf(Request $request)
    {
        ...
        return view('backend.dc.pages.reports.pdf_missing', [
            'rows'     => $rows,
            'filters'  => $filters,
    ...
    public function exportSubmittedPdf(Request $request)
    {
        ...
        return view('backend.dc.pages.reports.pdf_submitted', [
            'rows'    => $rows,
            'filters' => $filters,
        ]);
    }
        ...
        return response()->streamDownload(function () use ($headers, $rows) {
            ...
            foreach ($rows as $row) {
                $escaped = array_map(function ($val) {
                    if ($val === null) return '';
                    // Don't trim — preserves \t prefix for phone numbers
    ...
    public function exportCsv(Request $request)
    {
        ...
        return $this->streamCsv('dc-stock-sales-' . now()->format('Y-m-d') . '.csv', $headers, $rows);
    }
    ...
    public function exportDifferenceCsv(Request $request)
    {
        ...
        if ($request->filled('thana_upazila')) $query->where('thana_upazila', $request->thana_upazila);
        if ($request->filled('company_id')) $query->whereHas('fillingStation', fn($q) => $q->where('company_id', $request->company_id));
        ...
        return $this->streamCsv('dc-difference-' . now()->format('Y-m-d') . '.csv', $headers, $rows);
    }
    ...
    public function exportMissingCsv(Request $request)
    {
        ...
        return $this->streamCsv('dc-missing-' . now()->format('Y-m-d') . '.csv', $headers, $rows);
    }
    ...
    public function exportSubmittedCsv(Request $request)
    {
        ...
        if ($request->filled('thana_upazila')) $query->where('thana_upazila', $request->thana_upazila);
        if ($request->filled('company_id'))    $query->whereHas('fillingStation', fn($q) => $q->where('company_id', $request->company_id));
        if ($request->filled('depot_id'))      $query->whereHas('fillingStation', fn($q) => $q->where('depot_id', $request->depot_id));
        if ($request->filled('station_id'))    $query->where('station_id', $request->station_id);
        ...
        return $this->streamCsv('dc-submitted-' . now()->format('Y-m-d') . '.csv', $headers, $rows);
    }

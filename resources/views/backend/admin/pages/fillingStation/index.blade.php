...
<style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    /* Header */
    .fs-header { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 28px; }
    .fs-header-left h2 { font-size: 1.65rem; font-weight: 800; color: #0f172a; margin: 0 0 4px; letter-spacing: -0.4px; }
    ...
    .btn-add:hover { color: #fff; text-decoration: none; transform: translateY(-1px); box-shadow: 0 6px 20px rgba(37,99,235,0.45); }
    ...
    .stat-card::after  { content:''; position:absolute; right:-16px; bottom:-16px; width:80px; height:80px; border-radius:50%; background:rgba(255,255,255,0.12); }
    .stat-card::before { content:''; position:absolute; right:18px; top:-20px; width:60px; height:60px; border-radius:50%; background:rgba(255,255,255,0.08); }
    .stat-card-label { font-size:0.75rem; font-weight:600; text-transform:uppercase; letter-spacing:0.06em; color:rgba(255,255,255,0.82); margin-bottom:8px; }
    .stat-card-value  { font-size:2.4rem; font-weight:800; color:#fff; line-height:1; letter-spacing:-1px; }
    .stat-card-icon   { position:absolute; right:18px; top:50%; transform:translateY(-50%); opacity:0.22; }
    ...
    /* Filter Card */
    .fs-filter-card { background:#fff; border-radius:14px; border:1px solid #e2e8f0; padding:16px 20px; margin-bottom:20px; box-shadow:0 1px 4px rgba(0,0,0,0.04); }
    .fs-filter-row  { display:flex; align-items:center; gap:12px; flex-wrap:wrap; }
    .fs-filter-label { display:flex; align-items:center; gap:6px; font-size:0.8rem; font-weight:700; color:#475569; letter-spacing:0.03em; text-transform:uppercase; white-space:nowrap; }
    .fs-filter-label svg { width:14px; height:14px; }
    .fs-input {
        ...
        transition:border-color 0.15s, box-shadow 0.15s; flex:1; min-width:160px;
    }
    .fs-input:focus { border-color:#2563eb; box-shadow:0 0 0 3px rgba(37,99,235,0.1); background:#fff; }
    .fs-input::placeholder { color:#94a3b8; }
    .fs-search-wrap { position:relative; flex:2; min-width:200px; }
    .fs-search-wrap svg { position:absolute; left:12px; top:50%; transform:translateY(-50%); width:15px; height:15px; color:#94a3b8; }
    .fs-search-wrap input { padding-left:36px; }
    .btn-clear {
        display:inline-flex; align-items:center; gap:6px;
        background:#f1f5f9; color:#475569; border:1.5px solid #e2e8f0;
        border-radius:9px; padding:9px 16px; font-size:0.82rem; font-weight:600;
    ...
    /* Table Card */
    .fs-table-card { background:#fff; border-radius:16px; border:1px solid #e2e8f0; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.05); }
    .fs-table { width:100%; border-collapse:collapse; }
    .fs-table thead tr { background:#f8fafc; border-bottom:2px solid #e2e8f0; }
    .fs-table thead th { font-size:0.70rem; font-weight:800; color:#64748b; letter-spacing:0.08em; text-transform:uppercase; padding:13px 16px; white-space:nowrap; }
    .fs-table tbody tr { border-bottom:1px solid #f1f5f9; transition:background 0.12s; }
    ...
    .row-index { width:28px; height:28px; border-radius:8px; background:#f1f5f9; color:#64748b; font-size:0.75rem; font-weight:700; display:inline-flex; align-items:center; justify-content:center; }
    ...
    .badge-code   { display:inline-block; padding:4px 10px; border-radius:7px; border:1.5px solid #bfdbfe; background:#eff6ff; color:#1d4ed8; font-size:0.72rem; font-weight:800; letter-spacing:0.04em; }

    .badge-status { display:inline-flex; align-items:center; gap:5px; padding:5px 12px; border-radius:20px; font-size:0.76rem; font-weight:700; white-space:nowrap; }
    ...
    .badge-fuel { display:inline-block; padding:3px 8px; border-radius:5px; background:#f1f5f9; color:#475569; font-size:0.70rem; font-weight:700; margin:1px 2px; border:1px solid #e2e8f0; }

    .actions-cell { display:flex; align-items:center; gap:4px; }
    .action-btn { display:inline-flex; align-items:center; justify-content:center; width:34px; height:34px; border-radius:9px; border:none; cursor:pointer; background:transparent; transition:background 0.15s; text-decoration:none; }
    ...
    .fs-alert { border-radius:11px; padding:12px 18px; margin-bottom:18px; font-size:0.875rem; display:flex; align-items:center; gap:10px; }
    ...
    .empty-row td { text-align:center; color:#94a3b8; padding:56px 20px; font-size:0.875rem; }
    ...
    .fs-modal .modal-title { color: #fff; font-weight: 800; font-size: 1.05rem; letter-spacing: -0.2px; }
    .fs-modal .btn-close { filter: brightness(0) invert(1); opacity: 0.8; }
    ...
    .fs-modal .form-label { font-size: 0.8rem; font-weight: 600; color: #374151; margin-bottom: 5px; }
    .fs-modal .form-label .req { color: #ef4444; margin-left: 2px; }
    .fs-modal .form-control, .fs-modal .form-select {
    ...
    .fs-modal .form-control:focus, .fs-modal .form-select:focus {
        border-color: #2563eb; box-shadow: 0 0 0 3px rgba(37,99,235,0.1);
        background: #fff;
    }
    .fs-modal .invalid-feedback { font-size: 0.75rem; }
    ...
    .file-upload-label:hover { border-color: #2563eb; color: #2563eb; }
    .file-upload-label svg { width: 17px; height: 17px; flex-shrink: 0; }
    ...
    /* Spinner */
    .modal-loading { text-align: center; padding: 48px 0; color: #94a3b8; font-size: 0.9rem; }

    @media (max-width: 1100px) { .fs-stats { grid-template-columns: repeat(3, 1fr); } }
    @media (max-width: 700px)  { .fs-wrapper { padding: 20px 16px; } .fs-stats { grid-template-columns: repeat(2, 1fr); } .fs-header { flex-direction: column; gap: 14px; } }
</style>
@endpush
...
<div class="fs-wrapper">
    ...
    <div class="fs-stats">
        <div class="stat-card stat-total">
            ...
            <div class="stat-card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"><path d="M19.5 3h-15A2.5 2.5 0 0 0 2 5.5v13A2.5 2.5 0 0 0 4.5 21h15A2.5 2.5 0 0 0 22 18.5v-13A2.5 2.5 0 0 0 19.5 3zM12 17a5 5 0 1 1 0-10 5 5 0 0 1 0 10z"/></svg></div>
        </div>
        <div class="stat-card stat-active">
            <div class="stat-card-label">Active</div>
            <div class="stat-card-value">{{ $activeStations }}</div>
            <div class="stat-card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14l-4-4 1.41-1.41L11 13.17l6.59-6.59L19 8l-8 8z"/></svg></div>
        </div>
        <div class="stat-card stat-inactive">
            <div class="stat-card-label">Inactive</div>
            <div class="stat-card-value">{{ $inactiveStations }}</div>
            <div class="stat-card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"/></svg></div>
        </div>
        <div class="stat-card stat-govt">
            <div class="stat-card-label">Government</div>
            <div class="stat-card-value">{{ $govtStations }}</div>
            <div class="stat-card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L2 7v2h20V7L12 2zM4 11v7H2v2h20v-2h-2v-7h-2v7h-3v-7h-2v7H9v-7H7v7H5v-7H4z"/></svg></div>
        </div>
        <div class="stat-card stat-private">
            <div class="stat-card-label">Private</div>
            <div class="stat-card-value">{{ $privateStations }}</div>
            <div class="stat-card-icon"><svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"><path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z"/></svg></div>
        </div>
    </div>
        ...
        <table class="fs-table" id="stationTable">
            ...
            <tbody id="tableBody">
                ...
                <tr data-division="{{ strtolower($station->division ?? '') }}"
                    ...
                    <td>
                        {{ $station->district ?? '—' }}
                        @if($station->upazila)<br><span class="station-sub">{{ $station->upazila }}</span>@endif
                    </td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-header">
                <h5 class="modal-title" id="editModalLabel">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                         stroke="currentColor" stroke-width="2" style="width:18px;height:18px;margin-right:8px;vertical-align:-3px;">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487a2.1 2.1 0 1 1 2.97 2.97L7.5 19.79l-4 1 1-4 12.362-12.303z"/>
                    </svg>
...
<script>
    ...
    function submitEditForm() {
        ...
        .finally(() => {
            ...
            saveBtn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2" style="width:15px;height:15px;"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg> Save Changes`;
        });
    }
</script>
@endpush
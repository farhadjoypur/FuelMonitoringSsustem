...
.db-wrap   { background: var(--bg); min-height: 100vh; padding: 28px; }
.db-title  { font-size: 22px; font-weight: 800; color: var(--text); margin: 0 0 3px; letter-spacing: -.3px; }
.db-sub    { font-size: 13px; color: var(--muted); margin-bottom: 26px; }

/* grids */
.g-5  { display: grid; grid-template-columns: repeat(5,1fr); gap: 16px; margin-bottom: 24px; }
.g-3  { display: grid; grid-template-columns: repeat(3,1fr); gap: 16px; margin-bottom: 24px; }
.g-4  { display: grid; grid-template-columns: repeat(4,1fr); gap: 16px; margin-bottom: 24px; }
.g-22 { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px; }

.section-title { font-size: 14px; font-weight: 700; color: var(--text); margin: 0 0 14px; text-align: center; }

/* stat card */
.stat-card { background: var(--white); border-radius: var(--radius); border: 1px solid var(--border); box-shadow: var(--shadow); padding: 20px 18px; position: relative; }
.stat-card .sc-label { font-size: 11px; font-weight: 600; color: var(--muted); text-transform: uppercase; letter-spacing: .5px; }
.stat-card .sc-value { font-size: 26px; font-weight: 800; color: var(--text); letter-spacing: -.5px; line-height: 1.1; margin: 6px 0; }
.stat-card .sc-footer { font-size: 12px; color: var(--muted); }
.stat-card .sc-footer .up   { color: var(--green); font-weight: 600; }
.stat-card .sc-footer .down { color: var(--octane); font-weight: 600; }
.sc-alert  { display: flex; align-items: center; gap: 5px; font-size: 12px; font-weight: 700; color: var(--octane); margin: 4px 0; }
.sc-icon { position: absolute; top: 18px; right: 18px; width: 44px; height: 44px; border-radius: 11px; display: flex; align-items: center; justify-content: center; font-size: 18px; color: #fff; }
...
.sc-icon.red    { background: var(--octane); }

/* fuel cards */
.fuel-card  { border-radius: var(--radius); border: 1px solid transparent; padding: 20px 22px; display: flex; align-items: center; gap: 16px; }
.fuel-card.petrol { background: #f0fdf4; border-color: #bbf7d0; }
.fuel-card.diesel { background: #fff7ed; border-color: #fed7aa; }
.fuel-card.octane { background: #fef2f2; border-color: #fecaca; }
.fuel-icon  { width: 52px; height: 52px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 22px; color: #fff; flex-shrink: 0; }
.fuel-icon.p { background: var(--petrol); }
.fuel-icon.d { background: var(--diesel); }
.fuel-icon.o { background: var(--octane); }
.fuel-info .fi-val  { font-size: 26px; font-weight: 800; letter-spacing: -.5px; line-height: 1; }
.fuel-info .fi-name { font-size: 13px; font-weight: 500; margin-top: 3px; }
.fuel-card.petrol .fi-val, .fuel-card.petrol .fi-name { color: var(--petrol); }
.fuel-card.diesel .fi-val, .fuel-card.diesel .fi-name { color: var(--diesel); }
.fuel-card.octane .fi-val, .fuel-card.octane .fi-name { color: var(--octane); }

/* summary cards */
.sum-card { border-radius: var(--radius); padding: 24px 22px; color: #fff; position: relative; overflow: hidden; }
.sum-card::before { content: ''; position: absolute; top: -20px; right: -20px; width: 90px; height: 90px; border-radius: 50%; background: rgba(255,255,255,.1); }
...
.sum-card.orange-c { background: linear-gradient(135deg, #ea580c, #c2410c); }
.sum-card .sm-icon { font-size: 24px; margin-bottom: 12px; opacity: .9; }
.sum-card .sm-val  { font-size: 36px; font-weight: 800; letter-spacing: -1px; line-height: 1; }
.sum-card .sm-name { font-size: 13px; opacity: .85; margin-top: 4px; font-weight: 500; }
.sum-card .sm-trend{ font-size: 12px; opacity: .8; margin-top: 10px; display: flex; align-items: center; gap: 4px; }
...
.cc-head i { color: var(--blue); font-size: 15px; }
.cc-title { font-size: 14px; font-weight: 700; color: var(--text); }
.cc-select { margin-left: auto; border: 1px solid var(--border); border-radius: 7px; padding: 5px 10px; font-size: 12px; color: var(--text); background: var(--bg); }
...
/* depot table */
.depot-table-card { padding: 0; }
.dt-head { padding: 18px 22px 14px; display: flex; align-items: center; gap: 9px; border-bottom: 1px solid var(--border); }
.dt-head i { color: var(--blue); }
.dt-title { font-size: 14px; font-weight: 700; color: var(--text); }
.dep-table { width: 100%; border-collapse: collapse; }
.dep-table thead th { padding: 10px 18px; font-size: 10px; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: .5px; background: #fafbfc; border-bottom: 1px solid var(--border); text-align: left; }
.dep-table tbody td { padding: 13px 18px; font-size: 13px; color: var(--text); border-bottom: 1px solid #f5f6fa; }
.dep-table tbody tr:last-child td { border-bottom: none; }
.dep-table tbody tr:hover td { background: #fafbfc; }
.depot-name { font-weight: 700; }
.util-wrap  { display: flex; align-items: center; gap: 10px; }
.util-bar   { flex: 1; height: 6px; background: #e8ecf0; border-radius: 3px; overflow: hidden; min-width: 80px; }
...
.util-fill.low    { background: #22c55e; }
.util-fill.zero   { background: #e2e8f0; }
.util-pct  { font-size: 12px; font-weight: 700; color: var(--text); min-width: 34px; }
.badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; }
.badge.active   { background: #dcfce7; color: var(--green); }
.badge.inactive { background: #fee2e2; color: var(--octane); }

/* activities */
.activity-card { padding: 0; }
.ac-head   { padding: 18px 22px 14px; display: flex; align-items: center; gap: 9px; border-bottom: 1px solid var(--border); }
.ac-head i { color: var(--purple); }
.ac-title  { font-size: 14px; font-weight: 700; color: var(--text); }
.act-item  { display: flex; align-items: flex-start; gap: 14px; padding: 14px 22px; border-bottom: 1px solid #f5f6fa; }
.act-item:last-child { border-bottom: none; }
.act-dot { width: 34px; height: 34px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 13px; flex-shrink: 0; }
...
.act-dot.red    { background: #fee2e2; color: var(--octane); }
        ...
        <div class="stat-card">
            ...
            <div class="sc-footer">
                @if($receivedChangePct >= 0)
                    <span class="up"><i class="fa-solid fa-arrow-trend-up fa-xs"></i> +{{ $receivedChangePct }}%</span>
                @else
                    <span class="down"><i class="fa-solid fa-arrow-trend-down fa-xs"></i> {{ $receivedChangePct }}%</span>
                @endif
                &nbsp;vs last month
            </div>
        </div>
    ...
    <div class="g-22">
        ...
        <div class="card chart-card">
            ...
            <div class="chart-legend" style="margin-top:18px">
                <div class="legend-item"><span class="legend-dot" style="background:#1e6fa8"></span>Diesel: {{ number_format($totalDiesel) }} L</div>
                <div class="legend-item"><span class="legend-dot" style="background:#2d9748"></span>Petrol: {{ number_format($totalPetrol) }} L</div>
                <div class="legend-item"><span class="legend-dot" style="background:#f97316"></span>Octane: {{ number_format($totalOctane) }} L</div>
                @if($totalOthers > 0)
                <div class="legend-item"><span class="legend-dot" style="background:#c0392b"></span>Others: {{ number_format($totalOthers) }} L</div>
                @endif
            </div>
        </div>
        ...
        <div class="card depot-table-card">
            ...
            <table class="dep-table">
                ...
                <tbody>
                    ...
                    @endphp
                    <tr>
                        <td><div class="depot-name">{{ $depot['name'] }}</div></td>
                        <td><span style="font-size:12px;color:var(--muted)">{{ $depot['district'] }}</span></td>
                        <td>{{ $depot['capacity'] }}</td>
                        <td>
                            <div class="util-wrap">
                                <div class="util-bar">
                                    <div class="util-fill {{ $fillClass }}" style="width:{{ $u }}%"></div>
                                </div>
                    ...
                    @empty
                    <tr><td colspan="5"><div class="no-data">কোনো Depot পাওয়া যায়নি।</div></td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
...
new Chart(document.getElementById('fuelPieChart'), {
    type: 'pie',
    data: {
        labels: ['Diesel', 'Petrol', 'Octane'@if($totalOthers > 0), 'Others'@endif],
        datasets: [{
            data: [
                {{ $totalDiesel }},
                {{ $totalPetrol }},
                {{ $totalOctane }}
                @if($totalOthers > 0), {{ $totalOthers }} @endif
            ],
...
new Chart(document.getElementById('companyPieChart'), {
    ...
    options: {
        ...
        plugins: {
            legend: { display: false },
            tooltip: {
                callbacks: { label: ctx => ` ${ctx.label}: ${ctx.parsed} Stations` }
            }
        }
    }
});

...
@section('content')
    <div class="container-fluid p-4">
        ...
        <form id="profileUpdateForm" action="{{ route('tag-officer.profile.update', auth()->user()->id) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <div class="card profile-card h-100 p-5 text-center shadow-sm">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" name="photo" id="photoInput" class="d-none" accept="image/*"
                                data-url="{{ route('tag-officer.profile.update', auth()->user()->id) }}">
    ...
    <script>
        $('#photoInput').on('change', function() {
            ...
            if (file) {
                ...
                $.ajax({
                    url: "{{ route('tag-officer.profile.update', auth()->user()->id) }}",
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,

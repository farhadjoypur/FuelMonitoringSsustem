...
<style>
    ...
    .custom-card { background: #fff; border-radius: 10px; border: 1px solid #e2e8f0; margin-bottom: 20px; overflow: hidden; }

    .filter-section { background: #fff; border-radius: 10px; border: 1px solid #e2e8f0; padding: 18px 20px; margin-bottom: 20px; }
    .filter-section label { font-size: 11px; font-weight: 700; color: var(--dark); text-transform: uppercase; letter-spacing: .4px; margin-bottom: 5px; display: block; }
    .filter-section .form-control { font-size: 13px; border-color: #e2e8f0; border-radius: 6px; color: var(--dark); }
    .filter-section .form-control:focus { border-color: var(--primary); box-shadow: 0 0 0 2px rgba(37,99,235,.1); outline: none; }

    .btn-add { background: var(--primary); color: #fff; padding: 10px 20px; border-radius: 7px; font-weight: 600; border: none; text-decoration: none; font-size: 13px; }
    .btn-add:hover { background: #1d4ed8; color: #fff; }
    .btn-add i { margin-right: 6px; }
    .btn-filter { background: var(--primary); color: #fff; border: none; border-radius: 6px; padding: 9px 18px; font-size: 13px; font-weight: 600; }
    .btn-reset { background: #f1f5f9; color: var(--dark); border: 1px solid #e2e8f0; border-radius: 6px; padding: 9px 13px; text-decoration: none; font-size: 13px; }
    .btn-reset:hover { background: #e2e8f0; }

    .table-custom thead th { background: #f8fafc; color: var(--dark); font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .4px; border-bottom: 2px solid #e2e8f0; padding: 12px 14px; white-space: nowrap; }
    .table-custom tbody td { padding: 11px 14px; font-size: 13px; border-bottom: 1px solid #f1f5f9; vertical-align: middle; color: var(--dark); }
    .table-custom tbody tr:hover td { background: #f8fafc; }

    .fuel-type { font-size: 12px; font-weight: 700; color: var(--dark); display: flex; align-items: center; gap: 7px; }
    .fuel-dot { width: 7px; height: 7px; border-radius: 50%; background: var(--primary); display: inline-block; flex-shrink: 0; }
    .fuel-dot.muted { background: #cbd5e1; }

    .station-name { font-weight: 700; font-size: 13px; color: var(--dark); }
    .station-sub  { font-size: 11px; color: #94a3b8; margin-top: 2px; }
    .station-sub i { margin-right: 3px; }

    .diff-neg  { color: #dc2626; font-weight: 700; }
    .diff-pos  { color: #16a34a; font-weight: 700; }
    .diff-zero { color: #94a3b8; }

    .action-wrap { display: flex; flex-direction: column; gap: 5px; align-items: stretch; min-width: 80px; }
    .btn-act { display: inline-flex; align-items: center; justify-content: center; gap: 5px; padding: 5px 10px; border-radius: 5px; font-size: 12px; font-weight: 600; text-decoration: none; border: none; cursor: pointer; width: 100%; }
    .btn-act-view { background: #f1f5f9; color: var(--dark); border: 1px solid #e2e8f0; }
    .btn-act-edit { background: var(--primary); color: #fff; }
    .btn-act-del  { background: #fff; color: #dc2626; border: 1px solid #fecaca; }
    .btn-act-view:hover { background: #e2e8f0; color: var(--dark); }
    ...
    .alert-danger  { background: #fef2f2; border: 1px solid #fecaca; color: #991b1b; border-radius: 8px; padding: 11px 16px; margin-bottom: 18px; font-size: 13px; display: flex; align-items: center; gap: 8px; }

    .empty-state { text-align: center; padding: 50px 20px; }
    .empty-state i { font-size: 34px; color: #cbd5e1; margin-bottom: 12px; display: block; }
    .empty-state p { font-size: 14px; color: #94a3b8; margin-bottom: 16px; }

    .group-border td { border-bottom: 2px solid #e2e8f0 !important; }
    .pagination-wrap { padding: 14px 18px; border-top: 1px solid #f1f5f9; }
    .closing-val { font-weight: 700; color: var(--primary); }
</style>
@endpush
...
<div class="report-container">
    ...
    <div class="page-header">
        <h4><i class="fa-solid fa-gas-pump"></i> Fuel Report List</h4>
        ...
        <form method="GET" action="{{ route('fuel-reports.index') }}">
            <div class="row g-2 align-items-end">
                <div class="col-md-4">
                    ...
                    <input type="text" name="station_name" class="form-control" placeholder="Search station..." value="{{ request('station_name') }}">
                </div>
                <div class="col-md-3">
                ...
                <tbody>
                    @forelse($reports as $report)
                        <tr>
                            ...
                            <td class="{{ $report->petrol_difference < 0 ? 'diff-neg' : ($report->petrol_difference > 0 ? 'diff-pos' : 'diff-zero') }}">
                        ...
                        <tr>
                            ...
                            <td class="{{ $report->diesel_difference < 0 ? 'diff-neg' : ($report->diesel_difference > 0 ? 'diff-pos' : 'diff-zero') }}">
                        ...
                        <tr class="group-border">
                            ...
                            <td class="{{ $report->octane_difference < 0 ? 'diff-neg' : ($report->octane_difference > 0 ? 'diff-pos' : 'diff-zero') }}">

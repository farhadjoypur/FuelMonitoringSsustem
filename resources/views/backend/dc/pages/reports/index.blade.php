    ...
    <style>
        ...
        *::after {
        ...
        .btn-apply:hover {
            ...
            box-shadow: 0 6px 16px rgba(15, 76, 129, .3);
        }
                            ...
                            <select name="officer_id">
                                ...
                                @foreach ($officers as $officer)
                                    <option value="{{ $officer->id }}"
                                        {{ request('officer_id') == $officer->id ? 'selected' : '' }}>
                                        {{ $officer->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="filter-group">
                            ...
                            <select name="district">
                                ...
                                @foreach ($districts as $dist)
                                    <option value="{{ $dist }}"
                                        {{ request('district') == $dist ? 'selected' : '' }}>{{ $dist }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="filter-group">
            ...
            <div class="stat-cards">
                ...
                <div class="stat-card green">
                    ...
                    <div>
                        ...
                        <div class="stat-card-value">{{ number_format($stockReports->sum('diesel_closing_stock')) }} L
                        </div>
                    </div>
                </div>
                <div class="stat-card purple">
                    ...
                    <div>
                        ...
                        <div class="stat-card-value">{{ number_format($stockReports->sum('petrol_closing_stock')) }} L
                        </div>
                    </div>
                </div>
                <div class="stat-card orange">
                    ...
                    <div>
                        ...
                        <div class="stat-card-value">{{ number_format($stockReports->sum('octane_closing_stock')) }} L
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            @forelse($stockReports as $i => $r)
                                @php
                                    $totalClosing =
                                        $r->diesel_closing_stock + $r->petrol_closing_stock + $r->octane_closing_stock;
                                    if ($totalClosing <= 0) {
                                        $sClass = 'status-zero';
                                        $sLabel = 'Zero Stock';
                            ...
                            @empty
                                <tr class="empty-row">
                                    <td colspan="20">
                                        <i class="fa-solid fa-inbox" style="margin-right:7px;opacity:.4;"></i>No stock
                                        data found.
                                    </td>
                                </tr>
                            @endforelse
            ...
            <div class="stat-cards">
                <div class="stat-card blue">
                    ...
                    <div>
                        ...
                        <div class="stat-card-value">
                            {{ number_format($salesReports->sum('diesel_sales') + $salesReports->sum('petrol_sales') + $salesReports->sum('octane_sales')) }}
                            L
                        </div>
                    </div>
                </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr class="empty-row">
                                    <td colspan="12">
                                        <i class="fa-solid fa-inbox" style="margin-right:7px;opacity:.4;"></i>No sales
                                        data found.
                                    </td>
                                </tr>
                            @endforelse
                            @if ($salesReports->count() > 0)
                                <tr class="summary-row">
                                    ...
                                    <td>{{ number_format($salesReports->sum('diesel_sales') + $salesReports->sum('petrol_sales') + $salesReports->sum('octane_sales')) }}
                                        L</td>
                                </tr>
                            @endif
                        </tbody>
                        ...
                        <tbody>
                            @forelse($officerReports as $i => $assign)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="status-badge {{ $assign->status === 'active' ? 'status-active' : 'status-inactive' }}">
                            ...
                            @empty
                                <tr class="empty-row">
                                    <td colspan="9">
                                        <i class="fa-solid fa-inbox" style="margin-right:7px;opacity:.4;"></i>No tag
                                        officer data found.
                                    </td>
                                </tr>
                            @endforelse
                ...
                <form method="GET" action="{{ route('dc.reports.index') }}">
                    ...
                    <div class="filter-grid">
                        ...
                        <div class="filter-group">
                            ...
                            <input type="number" name="min_diff" value="{{ request('min_diff', 0) }}"
                                placeholder="e.g. 50">
                        </div>
                        <div class="filter-apply-row" style="grid-column: 3/5;">
                        ...
                        <tbody>
                            @forelse($diffReports as $i => $r)
                                @php
                                    $petrolDiffPct =
                                        $r->petrol_received > 0
                                            ? round(($r->petrol_difference / $r->petrol_received) * 100, 2)
                                            : 0;
                                    $dieselDiffPct =
                                        $r->diesel_received > 0
                                            ? round(($r->diesel_difference / $r->diesel_received) * 100, 2)
                                            : 0;
                                    $octaneDiffPct =
                                        $r->octane_received > 0
                                            ? round(($r->octane_difference / $r->octane_received) * 100, 2)
                                            : 0;
                                ...
                                <tr>
                                    ...
                                    <td class="{{ $diffClass($r->petrol_difference) }}">
                                        {{ $r->petrol_difference > 0 ? '+' : '' }}{{ number_format($r->petrol_difference) }}
                                        L
                                    </td>
                                    <td class="{{ $petrolDiffPct > 0 ? 'diff-positive' : 'diff-zero' }}">
                                        {{ $petrolDiffPct }}%</td>
                                    <td class="{{ $diffClass($r->diesel_difference) }}">
                                        {{ $r->diesel_difference > 0 ? '+' : '' }}{{ number_format($r->diesel_difference) }}
                                        L
                                    </td>
                                    <td class="{{ $dieselDiffPct > 0 ? 'diff-positive' : 'diff-zero' }}">
                                        {{ $dieselDiffPct }}%</td>
                                    <td class="{{ $diffClass($r->octane_difference) }}">
                                        {{ $r->octane_difference > 0 ? '+' : '' }}{{ number_format($r->octane_difference) }}
                                        L
                                    </td>
                                    <td class="{{ $octaneDiffPct > 0 ? 'diff-positive' : 'diff-zero' }}">
                                        {{ $octaneDiffPct }}%</td>
                            ...
                            @empty
                                <tr class="empty-row">
                                    <td colspan="12">
                                        <i class="fa-solid fa-inbox" style="margin-right:7px;opacity:.4;"></i>No
                                        difference data found.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div class="stat-cards">
                ...
                <div class="stat-card red">
                    ...
                    <div>
                        ...
                        <div class="stat-card-value">
                            {{ number_format($dueSalesReports->sum(fn($r) => max(0, $r->diesel_sales - ($r->diesel_received + $r->diesel_prev_stock)))) }}
                            L
                        </div>
                    </div>
                </div>
                <div class="stat-card orange">
                    ...
                    <div>
                        ...
                        <div class="stat-card-value">
                            {{ number_format($dueSalesReports->sum(fn($r) => max(0, $r->petrol_sales - ($r->petrol_received + $r->petrol_prev_stock)))) }}
                            L
                        </div>
                    </div>
                </div>
                <div class="stat-card purple">
                    ...
                    <div>
                        ...
                        <div class="stat-card-value">
                            {{ number_format($dueSalesReports->sum(fn($r) => max(0, $r->octane_sales - ($r->octane_received + $r->octane_prev_stock)))) }}
                            L
                        </div>
                    </div>
                </div>
                        ...
                        <tbody>
                            @forelse($dueSalesReports as $i => $r)
                                ...
                                <tr>
                                    ...
                                    <td class="sold-val">{{ number_format($r->diesel_sales) }} L</td>
                                    <td class="{{ $dDue > 0 ? 'diff-positive' : 'diff-zero' }}"
                                        style="font-weight:700;">
                                        {{ $dDue > 0 ? number_format($dDue) . ' L' : '—' }}
                                    </td>
                                    <td>{{ number_format($pAvail) }} L</td>
                                    <td class="sold-val">{{ number_format($r->petrol_sales) }} L</td>
                                    <td class="{{ $pDue > 0 ? 'diff-positive' : 'diff-zero' }}"
                                        style="font-weight:700;">
                                        {{ $pDue > 0 ? number_format($pDue) . ' L' : '—' }}
                                    </td>
                                    <td>{{ number_format($oAvail) }} L</td>
                                    <td class="sold-val">{{ number_format($r->octane_sales) }} L</td>
                                    <td class="{{ $oDue > 0 ? 'diff-positive' : 'diff-zero' }}"
                                        style="font-weight:700;">
                            ...
                            @empty
                                <tr class="empty-row">
                                    <td colspan="14">
                                        <i class="fa-solid fa-inbox" style="margin-right:7px;opacity:.4;"></i>No due sales
                                        data found.
                                    </td>
                                </tr>
                            @endforelse
